...
const ArtemisTracker3D: FC = () => {
  ...
  useEffect(() => {
    ...
    scn.add(new THREE.AmbientLight(0x1a2030, 0.35));
    ...
    const makeStars = (count: number, size: number, spread: number): void => {
      ...
      scn.add(new THREE.Points(g, new THREE.PointsMaterial({ color: 0xffffff, size, sizeAttenuation: true, transparent: true, opacity: 0.8 })));
    };
    ...
    scn.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(tPts), new THREE.LineBasicMaterial({ color: 0x3377aa, transparent: true, opacity: 0.18 })));

    const cLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints(tPts.slice(0, 2)), new THREE.LineBasicMaterial({ color: 0x5599dd, transparent: true, opacity: 0.65 }));
  ...
  return (
    <div style={{ background: "#030610", height: "100vh", fontFamily: "'IBM Plex Mono','JetBrains Mono',monospace", color: "#c8d3e0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0}
        .sc{background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.05);border-radius:8px;padding:8px 14px}
        .lbl{font-size:10px;color:#4b5c72;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:2px}
        input[type=range]{-webkit-appearance:none;appearance:none;background:transparent;cursor:pointer;height:20px}
        input[type=range]::-webkit-slider-runnable-track{height:3px;background:linear-gradient(90deg,#1e40af,#3b82f6,#eab308,#3b82f6);border-radius:2px;opacity:.3}
        ...
        input[type=range]::-moz-range-track{height:3px;background:linear-gradient(90deg,#1e40af,#eab308);border-radius:2px;opacity:.3}
      ...
      <div style={{ padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,.05)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 12, color: "#4b5c72", fontWeight: 500 }}>DAY {day}</span>
          <span style={{ fontSize: 9, color: "#1e3a5f", background: "rgba(59,130,246,0.08)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(59,130,246,0.12)" }}>NASA OEM DATA</span>
        </div>
      ...
      <div style={{ padding: "6px 20px", display: "flex", alignItems: "center", gap: 8, borderBottom: "1px solid rgba(255,255,255,.03)", flexShrink: 0, flexWrap: "wrap" }}>
        {([{ l: "⏪", s: -80000 }, { l: "◁", s: -15000 }, { l: "⏸", s: 0 }, { l: "▷", s: 15000 }, { l: "⏩", s: 80000 }] as const).map(b => (
          <button key={b.l} onClick={() => { if (live) { setTOver(Date.now()); setLive(false); } setSpeed(b.s); }}
            style={{ background: !live && speed === b.s ? "rgba(234,179,8,.12)" : "rgba(255,255,255,.03)", border: !live && speed === b.s ? "1px solid rgba(234,179,8,.25)" : "1px solid rgba(255,255,255,.05)", color: !live && speed === b.s ? "#eab308" : "#4b5c72", borderRadius: 5, padding: "4px 10px", fontSize: 14 }}>{b.l}</button>
        ))}
        <button onClick={goLive} style={{ background: live ? "rgba(34,197,94,.1)" : "rgba(255,255,255,.03)", border: live ? "1px solid rgba(34,197,94,.25)" : "1px solid rgba(255,255,255,.05)", color: live ? "#22c55e" : "#4b5c72", borderRadius: 5, padding: "4px 12px", fontSize: 11, fontWeight: live ? 700 : 400, letterSpacing: "1px" }}>● LIVE</button>
        <input type="range" min={-3600000} max={MISSION_DUR + 3600000} value={eNow - LAUNCH_UTC} onChange={onSlide} style={{ flex: 1, minWidth: 120 }} />
        <span style={{ fontSize: 10, color: "#3d4a5c", whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
          <span>{new Date(eNow).toUTCString().replace("GMT", "UTC")}</span>
          <span style={{ color: "#5a6a80" }}>{new Date(eNow).toLocaleString(undefined, { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", timeZoneName: "short" })}</span>
        </span>
      </div>

      <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
        <canvas ref={cvRef} style={{ width: "100%", height: "100%", display: "block", cursor: "grab" }} />
        <div style={{ position: "absolute", bottom: 8, left: 14, fontSize: 10, color: "#1e293b", pointerEvents: "none", letterSpacing: ".5px" }}>DRAG ORBIT · SCROLL ZOOM · RIGHT-DRAG PAN</div>
        ...
        <div style={{ position: "absolute", top: 12, right: 12, display: "flex", flexDirection: "column", gap: 4 }}>
          {([{ id: "full" as CamMode, label: "FULL MISSION", icon: "◎" }, { id: "orion" as CamMode, label: "FOLLOW ORION", icon: "△" }, { id: "flyby" as CamMode, label: "FLYBY VIEW", icon: "⟐" }, { id: "moon" as CamMode, label: "MOON", icon: "◑" }, { id: "earth" as CamMode, label: "EARTH", icon: "◉" }]).map(v => (
            <button key={v.id} onClick={() => setCamMode(v.id)}
              style={{ display: "flex", alignItems: "center", gap: 8, background: camMode === v.id ? "rgba(234,179,8,.12)" : "rgba(3,6,16,.7)", backdropFilter: "blur(8px)", border: camMode === v.id ? "1px solid rgba(234,179,8,.3)" : "1px solid rgba(255,255,255,.06)", color: camMode === v.id ? "#eab308" : "#6b7a94", borderRadius: 6, padding: "7px 12px", fontSize: 11, fontFamily: "inherit", letterSpacing: ".5px", textAlign: "left" as const, width: 155, transition: "all .2s ease" }}>
        ...
        <div style={{ position: "absolute", top: 12, left: 12, display: "flex", flexDirection: "column", gap: 5 }}>
          <div style={{ background: "rgba(3,6,16,.75)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,.04)", borderRadius: 8, padding: "8px 14px" }}>
            <div className="lbl">EARTH DISTANCE</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#3b82f6", fontVariantNumeric: "tabular-nums" }}>{fmtD(dE)}</div>
            <div style={{ fontSize: 10, color: "#4b5c72", marginTop: 1 }}>{fmtD(dE * 0.621371).replace("km", "mi")}</div>
          </div>
          <div style={{ background: "rgba(3,6,16,.75)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,.04)", borderRadius: 8, padding: "8px 14px" }}>
            <div className="lbl">MOON DISTANCE</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#9ca3af", fontVariantNumeric: "tabular-nums" }}>{fmtD(dM)}</div>
            <div style={{ fontSize: 10, color: "#4b5c72", marginTop: 1 }}>{fmtD(dM * 0.621371).replace("km", "mi")}</div>
          </div>
        </div>
      </div>

      <div style={{ padding: "8px 20px", display: "flex", gap: 8, flexWrap: "wrap", borderTop: "1px solid rgba(255,255,255,.05)", flexShrink: 0 }}>
        <div className="sc" style={{ flex: 1, minWidth: 90 }}>
          ...
          <div style={{ height: 3, background: "rgba(255,255,255,.04)", borderRadius: 2, marginTop: 3, overflow: "hidden" }}><div style={{ height: "100%", width: `${mf * 100}%`, background: "linear-gradient(90deg,#1e40af,#eab308)", borderRadius: 2 }} /></div>
        </div>
        ...
        <div className="sc" style={{ flex: 1.5, minWidth: 200 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 3 }}>{crew.map(c => <span key={c.n} style={{ fontSize: 12 }}><span style={{ color: "#4b5c72" }}>{c.r}</span> <span style={{ color: "#c8d3e0" }}>{c.n}</span></span>)}</div>
        </div>
      </div>
    </div>
  );
